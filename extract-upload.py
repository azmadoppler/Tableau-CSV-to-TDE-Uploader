...
#csvfilenamemask = 'donothing.really' # needs to be passed in from php file, for now it just uploads everything

# sourcedir = 'C:\\Users\\USER\\Desktop\\Stelligence Intern\\tableau-uploader\\uploads' # need to double up the \\s | windows shares use like this '\\\\ryrobesxps\d$\' etc
# targetdir = 'C:\\Users\\USER\\Desktop\\Stelligence Intern\\tableau-uploader\\uploads' # can't be a share or UNC path
...
### TRYING TO USE INPUT PARAMETERS LIKE A SMART FUCKER ###
##def main(argv):
##   filemask = ''
##   try:
##      opts, args = getopt.getopt(argv,"hi:",["filemask="])
##   except getopt.GetoptError:
##      print 'extract-upload.py -i <filemask>'
##      sys.exit(2)
##   for opt, arg in opts:
##      if opt == '-h':
##         print 'extract-upload.py -i <filemask>'
##         sys.exit()
##      elif opt in ("-i", "--filemask"):
##         filemask = arg
##   csvfilenamemask = filemask
##   print 'csvfilemask is "', csvfilenamemask, '"'

csvfilenamemask = sys.argv[1]
...
for csvfilename in os.listdir("."):
   if csvfilename == csvfilenamemask:
...
# BEGIN MULTI FILE LOOP
        ...
        for t in range(numfields):
            ...
            if not dfields[t]:
                    fieldname = 'col' + str(i) #protect against when there are some missing column headers
            else:
                    fieldname = dfields[t]
        ...
        csvfile.close()
        print '] '
        print '('+str(intWithCommas(rowsinserted))+' rows)' # to clear out the row on command lin
